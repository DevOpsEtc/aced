...
##	source:		$ . ~/src/deploy/cloud/aws/os_sec.sh  ##
######################################################
...
ssh aws "sudo sed -i \
  ...
  -e '$ a\AllowUsers $deployUser ubuntu' \
  -e '$ a\DebianBanner no' \
  -e '$ a\MaxAuthTries 1' \
  -e '$ a\MaxSessions 2' \
  -e '$ a\DebianBanner no' \
...
echo -e "\n$green \bUpdating ssh connection alias..."
sed -i \
  -e "s/User ubuntu/User $deployUser/" \
  -e 's/Port 22/Port 222/' \
	~/.ssh/config
...
echo -e "\n$green \bDeleting variables..."
unset sshPath sshName sshSrvIp sshSrvNm sshSrvUsr
#
# iptables
# fail2ban
